document.addEventListener('DOMContentLoaded', function(){ 
	var sliders = document.getElementsByClassName('jsgc-hook__slider');
	for (var item of sliders) {
		var slider = window.gcSlider({
			slider: item
		});
	}
});
window.gcSlider = function( options ) {
	options = extend({
		slider: '',
	}, options || {} );
	var container = options.slider.querySelector( '.gc-slider__inner' ),
		controlsContainer = options.slider.querySelector( '.gc-slider-controls' ),
		prevButton = controlsContainer.children[0],
        nextButton = controlsContainer.children[1],
		thumbsContainer = options.slider.querySelector( '.gc-slider-thumbs__row' ),
	...
	function setSlidePositions () {
		...
		thumbsContainer.children[0].classList.add('gc-slider-thumbs-item--active');
	}
	...
	function animateSlide (number, isOut) {
		...
		if(isOut){
			thumbsContainer.children[number].classList.remove('gc-slider-thumbs-item--active');
		} else {
			thumbsContainer.children[number].classList.add('gc-slider-thumbs-item--active');
		}
	}
